...
class NetworkMetaManager:
    ...
    def _subprocess_call(self, command_list):
        ...
        try:
            ...
            self.logger.trace('_subprocess_call: Subprocess successful.')
        except subprocess.CalledProcessError as process_error:
            # Eat exceptions because it will properly return False this way.
            self.logger.error('Subprocess call failed!')

        self.logger.trace('_subprocess_call: "%s" returned %s.' % (' '.join(command_list), exit_code))
    ...
    def is_connected(self, network_name):
        ...
        self.logger.debug('is_connected: Checking connectivity for %s.' % network_name)
        ...
        if(network_name in subprocess.check_output(nmcli_command)):
            ...
            self.logger.debug('is_connected: Not connected to %s.' % network_name)
    ...
    def get_interface_ip(self, network_name):
        ...
        # TODO: compare IP address to regex to make sure it at least looks
        #         like an IP.  Not something to worry about, just good practice.
